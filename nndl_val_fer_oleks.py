# -*- coding: utf-8 -*-
"""NNDL_Val_Fer_Oleks.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1HZpX3L3-oeJ3ZKjbWiWHSoNX-sS2rDxh
...
class NeuralNetwork:
    ...
    def forward_propagation(self, input_data):
        ...
        Args:
            input_data (np.ndarray): Matrice di input di forma (num_esempi x 10),
                                    dove ogni riga rappresenta un esempio.

        Returns:
            np.ndarray: Output finale della rete neurale. Se `self.softmax` è True,
...
def k_fold_cross_validation(X, y, k, epochs, batch_size, input_size, hidden_size, output_size, learning_rate, eta_minus=0.5, eta_plus=1.2, delta_min=1e-6, delta_max=50):
    ...
    # n_samples = len(X)
    # indices = np.arange(n_samples)  #Indici da 0 a n_samples (escluso)
    # np.random.shuffle(indices)
    ...
    # skf = sk.KFold(n_splits=k, shuffle=True, random_state=42)
    skf = sk.StratifiedKFold(n_splits=k, shuffle=True, random_state=53)

    splits = list(skf.split(X, y))
    # skf.split è una lista di coppie di array: rispettivamente il primo elemento è il training set dell'i-esima iterazione, il secondo elemento è il test set della stessa iterazione (Sono complementari)

    # Ciclo attraverso i fold
    for i in range(k):
        # Definisci i fold di test e validation
        test_fold = i  # Il fold corrente è usato per testing
        val_fold = (i + 1) % k  # Il prossimo fold è usato per validation
        ...
        fold_visual = ["T"] * k  # Default: tutti i fold sono per il training
        fold_visual[test_fold] = "E"  # Segna il fold di test
        fold_visual[val_fold] = "V"  # Segna il fold di validation
        print(f"Fold {i + 1}: " + " | ".join(fold_visual))
        print("-" * 50)

        # Creazione dei set di training, validation e test
        # Qui prendiamo il complemento dell'unione dei due set creati
        train_index = np.setdiff1d(
            np.arange(len(X)), np.concatenate((test_index, val_index)))

        # Check for overlap
        overlap_train_val = np.intersect1d(train_index, val_index)
        overlap_train_test = np.intersect1d(train_index, test_index)
        overlap_val_test = np.intersect1d(val_index, test_index)

        # if len(overlap_train_val) > 0:
        #     print(f"⚠️ Overlap detected between Train and Validation: {overlap_train_val}")
        # if len(overlap_train_test) > 0:
        #     print(f"⚠️ Overlap detected between Train and Test: {overlap_train_test}")
        # if len(overlap_val_test) > 0:
        #     print(f"⚠️ Overlap detected between Validation and Test: {overlap_val_test}")
        # if len(overlap_train_val) == 0 and len(overlap_train_test) == 0 and len(overlap_val_test) == 0:
        #     print("✅ No overlap detected between Train, Validation, and Test sets.")

        # Estrazione dei dati per i set
        ...
        print(
            f"y's shapes = {y_train.shape} , {y_validation.shape} and {y_test.shape}")

        print(f"type x train: {type(X_train)}")

        # Encoding one-hot dei label
        # sparse_output=False restituisce un ndarray one-hot denso (con gli 0 e un 1) ; categories='auto' riconosce automaticamente le categorie
        ...
        print(
            f"y's shapes = {y_train_onehot.shape} , {y_validation_onehot.shape} and {y_test_onehot.shape}")
        ...
        print(f"prints {y_validation} and the other {y_validation_onehot}")
        ...
        error = np.mean(nn.error_function(final_output_test,
                        y_test_onehot))  # Calcola l'errore
        errors.append(error)
        ...
        # classes = y_test
        classes = np.argmax(y_test_onehot, axis=1)  # Etichette reali
        accuracy = np.mean(predictions == classes)  # Accuratezza
...
def grid_search(X, y, k, epochs, batch_size, input_size, output_size, learning_rate, hyperparameters, plot_data=True, export_csv=True):
    """
      Esegue una ricerca a griglia sugli iperparametri per trovare la combinazione ottimale per allenare la rete neurale.
    ...
    # Lista di valori per eta_minus
    h_eta_minusses = hyperparameters["h_eta_minusses"]
    # Lista di valori per eta_plus
    h_eta_plusses = hyperparameters["h_eta_plusses"]
    # Lista di valori per il numero di neuroni nei layer nascosti
    h_neurons = hyperparameters["h_neurons"]

    # Inizializza la variabile per memorizzare il miglior errore
    # Inizializza il miglior errore con un valore molto alto
    best_error = float('inf')
    best_params = None  # Memorizzerà i migliori iperparametri

    all_errors = []  # Lista per memorizzare gli errori per ogni combinazione di parametri
    # Lista per memorizzare le accuratezze per ogni combinazione di parametri
    all_accuracies = []
    param_labels = []  # Lista per memorizzare le etichette dei parametri
    ...
    # print(f"  ➤ eta_minus: {best_params[0]}")
    # print(f"  ➤ eta_plus: {best_params[1]}")
    # print(f"  ➤ hidden_neurons: {best_params[2]}")
    # print(f"  ➤ Errore medio: {best_error:.4f}")
...
def random_search(X, y, k, epochs, batch_size, input_size, output_size, learning_rate, hyperparameters, n_iter=5, plot_data=True, export_csv=True):
    """
    Implementazione Random Search con k-fold cross-validation per ottimizzare iperparametri di SimpleNN.
    :param X: Dati di input
    :param y: Etichette target
    :param k: Numero di fold per la cross-validation
    :param epochs: Numero di epoche
    :param batch_size: Dimensione del batch
    :param input_size: Numero di feature in input
    :param output_size: Numero di classi in output
    :param learning_rate: Tasso di apprendimento
    :param hyperparameters: Dizionario con i range degli iperparametri
    :param n_iter: Numero di combinazioni casuali da testare
    :return: Lista di errori, accuratezze e migliori iperparametri trovati
    """
    h_eta_minusses = hyperparameters["h_eta_minusses"]
    h_eta_plusses = hyperparameters["h_eta_plusses"]
    ...
    for _ in range(n_iter):
        ...
        # Aggiorna il miglior modello se l'errore è più basso
        # if mean_error < best_error:
        #     best_error = mean_error
        #     best_params = (eta_minus, eta_plus, hidden_neurons)
...
def plot_fold_errors(fold_errors, param_labels):
    """
    Disegna un grafico degli errori per ogni fold.
    :param fold_errors: Lista degli errori per ogni fold
    :param param_labels: Lista di etichette corrispondenti ai set di parametri
    """
    # Imposta il grafico
...
def plot_fold_accuracies(fold_accuracies, param_labels):
    """
    Disegna un grafico delle accuratezze per ogni fold.
    :param fold_accuracies: Lista delle accuratezze per ogni fold
    :param param_labels: Lista di etichette corrispondenti ai set di parametri
    """
    # Imposta il grafico
    plt.figure(figsize=(10, 6))
...
def main():
    ...
    # error_mean, error_std, accuracy_mean, accuracy_std = k_fold_cross_validation(X, y, k, epochs, batch_size, input_size, hidden_size, output_size, learning_rate)
    # print(f"Errore medio: {error_mean}, con deviazione standard: {error_std}")
    # print(f"Accuracy media: {accuracy_mean}, con deviazione standard: {accuracy_std}")
